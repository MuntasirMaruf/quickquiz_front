...
const QuestionsPage = ({ params }: PageProps) => {
    ...
    const [selectedQuestions, setSelectedQuestions] = useState<QuestionItem[]>([]); // local array
    ...
    const toggleQuestion = (q: QuestionItem) => {
        if (selectedQuestions.find((item) => item.id === q.id)) {
            ...
            if (selectedQuestions.length >= 10) return; // limit to 10
    ...
    const confirmQuestions = async () => {
        ...
        const questionsToSend = [...selectedQuestions]; // snapshot


        try {
            for (let i = 0; i < questionsToSend.length; i++) {
                const q = questionsToSend[i]; // now fixed
                const payload = {
                    exam_id: examid,
                    question_id_cq: q.id,
                    position: i + 1,
                ...
                await axios.post(
                    `${process.env.NEXT_PUBLIC_API_URL}/exam_question_ssc/create/exam_question`,
                    payload
                );
            }
        ...
        <div className="flex flex-col p-8 bg-gray-100 min-h-screen">
            ...
            <div className="bg-white rounded-lg shadow-lg p-8 w-full h-full overflow-y-auto space-y-6">
                {questions.map((item) => {
                    ...
                    return (
                        <div key={item.id} className={`mb-6 p-6 bg-gray-800 rounded-2xl shadow-lg text-white hover:shadow-2xl transition-shadow duration-300 ${isSelected ? "border-4 border-green-500" : ""}`}>
                            ...
                            <div className="flex justify-center mt-4">
                                <button
                                    className={`px-5 py-2 rounded-xl shadow-md transition-all duration-200 text-white ${isSelected ? "bg-red-500 hover:bg-red-600" : selectedQuestions.length >= 10 ? "bg-gray-400 cursor-not-allowed" : "bg-blue-500 hover:bg-blue-600 hover:scale-105"}`}
                                    onClick={() => toggleQuestion(item)}
                                    disabled={!isSelected && selectedQuestions.length >= 10}
                                >
